...
def process_data(raw_data, file_info, date_filters):
    ...
    return master.sort_values('商品コード').reset_index(drop=True), daily_master, totals, daily_raw
...
def get_product_daily_data(product_code, daily_raw, sales_df=None):
    ...
    result = pd.DataFrame({'日付': pd.date_range(start=min(all_dates), end=max(all_dates), freq='D')})
    
    for key in ['出荷実績', '特売予測', '製造予定', '製造実績']:
        if key in daily_raw:
            prod_df = daily_raw[key][daily_raw[key]['商品コード'] == product_code]
            if len(prod_df) > 0:
                result = result.merge(prod_df[['日付', key]], on='日付', how='left')
...
if IS_LOCAL:
    st.success("📁 ローカルモード: Zドライブから自動読み込み")
else:
...
if IS_LOCAL:
    ...
    st.sidebar.markdown("**必須（3つ）**")
    uploaded = {}
    uploaded['出荷実績'] = st.sidebar.file_uploader("① 出荷実績 (.xlsx)", type=['xlsx'], key='u1')
    uploaded['製造予定'] = st.sidebar.file_uploader("② 製造予定 (.xlsx)", type=['xlsx'], key='u2')
    uploaded['製造実績'] = st.sidebar.file_uploader("③ 製造実績 (.xlsx)", type=['xlsx'], key='u3')
    st.sidebar.markdown("**任意**")
    uploaded['特売情報'] = st.sidebar.file_uploader("④ 特売情報 (.csv)", type=['csv'], key='u4')
    uploaded['販売実績'] = st.sidebar.file_uploader("⑤ 販売実績 (.xlsx)", type=['xlsx'], key='u5')
    
    if uploaded['出荷実績'] and uploaded['製造予定'] and uploaded['製造実績']:
        raw_data, file_info, errors = load_data_from_upload(uploaded)
    else:
        st.warning("👈 サイドバーから必須ファイル3つをアップロード")
        st.markdown("""
        | # | ファイル | 形式 | 必須 |
        |---|---------|------|------|
        | 1 | 出荷実績 | .xlsx | ✅ |
...
for name in ['出荷実績', '特売情報', '製造予定', '製造実績']:
    ...
    if min_d is not None and max_d is not None and pd.notna(min_d) and pd.notna(max_d):
        with st.sidebar.expander(f"📁 {name}", expanded=True):
            st.caption(f"{info.get('file_name', '')} ({info.get('rows', 0):,}行)")
            c1, c2 = st.columns(2)
            with c1:
...
# データ処理
master, daily_master, totals, daily_raw = process_data(raw_data, file_info, date_filters)
...
# タブ
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["🚨 アラート", "📈 日別推移", "🔍 商品詳細", "📊 出荷分析", "🏭 製造分析", "📋 全商品"])
...
with tab1:
    ...
    if len(alerts_df) > 0:
        ...
        with c2:
            inf = st.multiselect("指示", ['⬆️ 製造増加', '⬇️ 製造減少検討', '⚠️ 製造遅れ対応'], default=['⬆️ 製造増加', '⬇️ 製造減少検討', '⚠️ 製造遅れ対応'])
...
with tab2:
    ...
    if len(daily_master) > 0:
        ...
        with c1:
            fig = go.Figure()
            fig.add_trace(go.Bar(x=daily_master['日付'], y=daily_master['出荷ズレ'], marker_color=np.where(daily_master['出荷ズレ'] >= 0, '#1f77b4', '#ff6b6b')))
            fig.add_hline(y=0, line_dash="dash")
            fig.update_layout(height=200, title="出荷ズレ")
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            fig = go.Figure()
            fig.add_trace(go.Bar(x=daily_master['日付'], y=daily_master['製造ズレ'], marker_color=np.where(daily_master['製造ズレ'] >= 0, '#2ca02c', '#ff6b6b')))
            fig.add_hline(y=0, line_dash="dash")
...
with tab3:
    ...
    with c1:
        search_term = st.text_input("🔎 商品検索", placeholder="商品名/コード")
    
    filtered_products = master[master['商品名'].str.contains(search_term, na=False) | master['商品コード'].str.contains(search_term, na=False)] if search_term else master
    
    if len(filtered_products) > 0:
        product_options = [f"{r['商品コード']} - {r['商品名']}" for _, r in filtered_products.iterrows()]
        with c2:
            selected = st.selectbox("商品選択", product_options)
        
        if selected:
            code = selected.split(" - ")[0]
            row = master[master['商品コード'] == code].iloc[0]
            
            st.markdown("---")
            st.markdown(f"#### 📦 {row['商品名']}")
            st.caption(f"コード: {row['商品コード']} / 規格: {row['規格']}")
            
            cols = st.columns(5)
            cols[0].metric("出荷予測", f"{row['出荷予測']:.0f}")
            cols[1].metric("出荷実績", f"{row['出荷実績']:.0f}")
            cols[2].metric("出荷ズレ率", f"{row['出荷ズレ率']:.1f}%")
            cols[3].metric("製造予定", f"{row['製造予定']:.0f}")
            cols[4].metric("製造実績", f"{row['製造実績']:.0f}")
            
            ins = row['製造指示']
            if "⬆️" in ins or "⚠️" in ins:
                st.error(f"**製造指示:** {ins}")
            elif "⬇️" in ins:
                st.warning(f"**製造指示:** {ins}")
            else:
                st.success(f"**製造指示:** {ins}")
            
            st.markdown("---")
            sales_df = raw_data.get('販売実績')
            prod_daily = get_product_daily_data(code, daily_raw, sales_df)
            
            if len(prod_daily) > 0:
                view_p = st.radio("表示", ["日別", "累計"], horizontal=True, key="pv")
                c1, c2 = st.columns(2)
                with c1:
                    fig = go.Figure()
                    if view_p == "日別":
                        fig.add_trace(go.Scatter(x=prod_daily['日付'], y=prod_daily['出荷予測'], name='予測', line=dict(color='lightblue')))
                        fig.add_trace(go.Scatter(x=prod_daily['日付'], y=prod_daily['出荷実績'], name='実績', line=dict(color='#1f77b4')))
                    else:
                        fig.add_trace(go.Scatter(x=prod_daily['日付'], y=prod_daily['出荷予測_累計'], name='予測累計', fill='tozeroy', line=dict(color='lightblue')))
                        fig.add_trace(go.Scatter(x=prod_daily['日付'], y=prod_daily['出荷実績_累計'], name='実績累計', line=dict(color='#1f77b4')))
                    fig.update_layout(height=250, title="出荷")
                    st.plotly_chart(fig, use_container_width=True)
                with c2:
                    fig = go.Figure()
                    if view_p == "日別":
                        fig.add_trace(go.Scatter(x=prod_daily['日付'], y=prod_daily['製造予定'], name='予定', line=dict(color='lightgreen')))
                        fig.add_trace(go.Scatter(x=prod_daily['日付'], y=prod_daily['製造実績'], name='実績', line=dict(color='#2ca02c')))
                    else:
                        fig.add_trace(go.Scatter(x=prod_daily['日付'], y=prod_daily['製造予定_累計'], name='予定累計', fill='tozeroy', line=dict(color='lightgreen')))
                        fig.add_trace(go.Scatter(x=prod_daily['日付'], y=prod_daily['製造実績_累計'], name='実績累計', line=dict(color='#2ca02c')))
                    fig.update_layout(height=250, title="製造")
                    st.plotly_chart(fig, use_container_width=True)
            else:
                st.info("日別データなし")
    else:
...
with tab4:
    ...
    if len(master) > 0:
        ...
        with c1:
            ...
            fig.update_layout(barmode='group', height=350, title="出荷TOP20", xaxis_tickangle=-45)
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            fig = px.histogram(master[master['出荷ズレ率'].between(-200, 200)], x='出荷ズレ率', nbins=50)
            fig.add_vline(x=th_ship, line_dash="dash", line_color="red")
            fig.add_vline(x=-th_ship, line_dash="dash", line_color="red")
            fig.update_layout(height=350, title="ズレ率分布")
            st.plotly_chart(fig, use_container_width=True)
...
with tab5:
    ...
    if len(master) > 0:
        ...
        with c1:
            ...
            fig.update_layout(barmode='group', height=350, title="製造TOP20", xaxis_tickangle=-45)
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            pf = master[(master['製造予定'] > 0) & master['製造ズレ率'].between(-200, 200)]
            fig = px.histogram(pf, x='製造ズレ率', nbins=50, color_discrete_sequence=['forestgreen'])
            fig.add_vline(x=-th_prod, line_dash="dash", line_color="red")
            fig.add_vline(x=th_prod, line_dash="dash", line_color="orange")
            fig.update_layout(height=350, title="ズレ率分布")
            st.plotly_chart(fig, use_container_width=True)
...
with tab6:
    ...
    cols_show = ['商品コード', '商品名', '出荷予測', '出荷実績', '出荷ズレ率', '製造予定', '製造実績', '製造ズレ率', '製造指示']
    display = master[[c for c in cols_show if c in master.columns]].copy()
    if search:
